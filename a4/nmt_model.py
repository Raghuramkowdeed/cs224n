...
class NMT(nn.Module):
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        tt = self.model_embeddings.target(target_padded)
        
        for tt_t in torch.split(tt, 1, dim=0):
            squeezed = torch.squeeze(tt_t) 
            tbar_t = torch.cat((squeezed, o_prev), dim=1) 
            dec_state, o_t, _ = self.step(tbar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        if enc_masks is not None:
            e_t.data.masked_fill_(enc_masks.bool(), -float('inf')) # mask the 0s with -inf, so e^x = 0
        ...
        a1_view = (alpha_t.size(0), 1, alpha_t.size(1))
        a_t = torch.bmm(alpha_t.view(*a1_view), enc_hiddens).squeeze(1)
